...
function atlagMatek(mezo){
  let osszeg = 0
  let atlag = 0
  let lista = FeltoltTablat()
  lista.forEach(elem=>{
    if (elem.Matematika + elem.Magyar >= document.getElementById("pontszam").value) {
      osszeg+=elem.Matematika
    }
  })
  atlag = Math.round(osszeg/lista.length, 2)
  mezo.innerText = atlag
}

function atlagMagyar(mezo){
  let osszeg = 0
  let atlag = 0
  let lista = FeltoltTablat()
  lista.forEach(elem=>{
...
function atlagOsszes(mezo){
  let osszeg = 0
  let atlag = 0
  let lista = FeltoltTablat()
  lista.forEach(elem=>{
    if (elem.Matematika + elem.Magyar >= document.getElementById("pontszam").value) {
      osszeg+=elem.Matematika+elem.Magyar
    }
  })
  atlag = Math.round(osszeg/lista.length, 2)
  mezo.innerText = atlag
...
function atlagKi(){
  let index=0
  let feliratok=["OM","Név","Matek","Magyar","Összesen"]
  let thList = document.getElementsByTagName("th")
  feliratok.forEach(felirat=>{
    thList[index].innerText=felirat
    index++
  })
