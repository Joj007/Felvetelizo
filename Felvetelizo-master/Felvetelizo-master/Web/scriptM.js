...
function Keres()
{
    ...
    let talalat = false
    let numberRegex = /^\d+$/;  
    let azon = document.getElementById("OM").value
    let csere = true
    document.getElementById("talalat").innerHTML = ""
    document.getElementById("tobbPontosok").innerText = ""
    document.getElementById("talalatTabla1").innerHTML = ""
    document.getElementById("talalatTabla2").innerHTML = ""
    if (azon.length != 11) {
        document.getElementById("auth").innerText = "Sikertelen keresés"
        document.getElementById("auth").classList = "sikertelen"
        alert("Az azonosító nem 11 karakter hosszú")
    }
    else if (!numberRegex.test(azon)) {
        document.getElementById("auth").innerText = "Sikertelen keresés"
        document.getElementById("auth").classList = "sikertelen"
        ...
        for (let adat of adatok) {
            if (adat.OM_Azonosito == azon) {
                document.getElementById("talalat").innerHTML =
                `
                  <div class="sor">
                    <div class="cim">Email:</div>
                    <div class="adat">${adat.Email}</div>
                  </div>
                  <div class="sor">
                    <div class="cim">Értesítési Címe:</div>
                    <div class="adat">${adat.ErtesitesiCime}</div>
                  </div>
                  <div class="sor">
                    <div class="cim">Magyar:</div>
                    <div class="adat">${adat.Magyar}</div>
                  </div>
                  <div class="sor">
                    <div class="cim">Matekmatika:</div>
                    <div class="adat">${adat.Matematika}</div>
                  </div>
                  <div class="sor">
                    <div class="cim">Neve:</div>
                    <div class="adat">${adat.Neve}</div>
                  </div>
                  <div class="sor">
                    <div class="cim">OM Azonosító:</div>
                    <div class="adat">${adat.OM_Azonosito}</div>
                  </div>
                  <div class="sor">
                    <div class="cim">Születési dátum:</div>
                    <div class="adat">${adat.SzuletesiDatum}</div>
                  </div>
                `
        ...
        if (talalat) {
          let tobbetElerokSzama = 0
            ...
            `
            for (let adat of adatok) {
              if (adat.Matematika + adat.Magyar > osszPont) {
                if (csere) {
                  document.getElementById("talalatTabla1").innerHTML +=
                  `
                    <tr>
                      <td>${adat.OM_Azonosito}</td>
                      <td>${adat.Neve}</td>
                    </tr>
                  `
                  csere = false
            ...
            document.getElementById("tobbPontosok").innerText = "Több pontot elérő diákok száma: " + tobbetElerokSzama
